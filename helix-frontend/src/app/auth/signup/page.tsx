...
export default function SignupPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (key) { try { localStorage.setItem('helix_api_key', key); } catch { /* ignore */ } }
      setSuccess(true);
      setTimeout(() => router.replace('/dashboard'), 1200);
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
      setError(msg || 'Failed to create account. Try a different email.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-sm">
        <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-2xl p-8 shadow-2xl">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-sm text-zinc-500 mt-1">Start understanding your codebase</p>
          </div>
          ...
          <AnimatePresence mode="wait">
            {success ? (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <div className="text-sm font-semibold text-green-300">Account created!</div>
                <div className="text-xs text-zinc-500">Redirecting to dashboard...</div>
              </motion.div>
            ) : (
              <motion.div key="form" className="space-y-4">
                <div>
                  ...
                  <input type="text" value={name} onChange={e => setName(e.target.value)}
                    onKeyDown={e => e.key === 'Enter' && submit()} placeholder="Harshita Yadav"
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPass ? 'text' : 'password'} value={password}
                      ...
                      onKeyDown={e => e.key === 'Enter' && submit()} placeholder="Min. 8 characters"
                  ...
                  {password.length > 0 && (
                    <div className="mt-2 flex items-center gap-2">
                      <div className="flex gap-1 flex-1">
                        {[1, 2, 3].map(i => (
                          <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= strength ? strengthColor[strength] : 'bg-[#1e1e2e]'}`} />
                        ))}
                      </div>
                      <span className={`text-[10px] ${strength === 1 ? 'text-red-400' : strength === 2 ? 'text-yellow-400' : 'text-green-400'}`}>
