...
export default function DocsPage() {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      const res = await generateDocs((selectedRepoId || '1'), tabType);
      const text: string = res.data?.content || res.data?.text || FALLBACK_CONTENT[tabType];
      ...
      // Fallback typewriter
      const text = FALLBACK_CONTENT[tabType];
      let i = 0;
      const words = text.split(' ');
      const iv = setInterval(() => {
        i++;
                ...
                <div className="max-w-3xl mx-auto">
                  {!currentContent && !generating ? (
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                      ...
                      <div className="text-sm text-zinc-600 mb-1">No content yet</div>
                      <div className="text-xs text-zinc-700">Click Generate to create {TABS[activeTab].label}</div>
                    </div>
                  ) : generating && !currentContent ? (
                    <div className="space-y-2 animate-pulse">
                      {Array.from({ length: 10 }).map((_, i) => (
                        <div key={i} className={`h-3 bg-[#1e1e2e] rounded`} style={{ width: `${60 + (i % 4) * 10}%` }} />
                      ))}
                    </div>
                  ) : (
