...
import { AlertTriangle, Info, XCircle, Play, CheckCircle2, RefreshCw, Shield, Code2 } from 'lucide-react';
...
const MOCK_SECURITY: SecurityFinding[] = [
  { severity: 'critical', file_path: 'src/auth.py', title: 'No rate limiting on login', description: 'Login endpoint accepts unlimited requests.', suggestion: 'Add rate limiting middleware to /auth/login.', line: 45 },
  { severity: 'high', file_path: 'src/utils.py', title: 'MD5 used for password hashing', description: 'MD5 is cryptographically broken.', suggestion: 'Switch to bcrypt or argon2.', line: 12 },
  { severity: 'medium', file_path: 'src/config.py', title: 'Hardcoded secret key', description: 'JWT secret appears hardcoded.', suggestion: 'Move to environment variable.', line: 8 },
  { severity: 'low', file_path: 'src/middleware.py', title: 'Exception swallowed silently', description: 'Errors caught but not logged.', suggestion: 'Add proper logging.', line: 34 },
];
const MOCK_SMELLS: CodeSmell[] = [
  { smell_type: 'God Class', node_name: 'UserService', file_path: 'src/services.py', suggestion: 'Split into UserQueryService and UserMutationService.', severity: 'warning' },
  { smell_type: 'Long Method', node_name: 'process_payment', file_path: 'src/payments/handler.py', suggestion: 'Extract subtasks into smaller helper methods.', severity: 'warning' },
  { smell_type: 'Dead Code', node_name: 'legacy_auth', file_path: 'src/auth.py', suggestion: 'Remove unused function to reduce maintenance burden.', severity: 'info' },
  { smell_type: 'Feature Envy', node_name: 'compute_user_score', file_path: 'src/analytics.py', suggestion: 'Move this method closer to the data it uses.', severity: 'info' },
  { smell_type: 'Magic Number', node_name: 'validate_token', file_path: 'src/auth.py', suggestion: 'Replace 86400 with a named constant TOKEN_TTL_SECONDS.', severity: 'info' },
];
...
const ANALYSIS_STEPS = ['Parsing files...', 'Analysing complexity...', 'Checking security...', 'Detecting smells...', 'Generating report...'];
...
const TABS = [
  { label: 'Health Score', icon: CheckCircle2 },
  { label: 'Security', icon: Shield },
  { label: 'Code Smells', icon: Code2 },
];
...
export default function AnalysisPage() {
  ...
  const loadAll = useCallback(() => {
    Promise.allSettled([getHealth((selectedRepoId || '1')), getSecurity((selectedRepoId || '1')), getSmells((selectedRepoId || '1'))])
      .then(([h, s, sm]) => {
        setHealth(h.status === 'fulfilled' ? h.value.data || MOCK_HEALTH : MOCK_HEALTH);
        setSecurity(s.status === 'fulfilled' ? s.value.data?.findings || MOCK_SECURITY : MOCK_SECURITY);
        setSmells(sm.status === 'fulfilled' ? sm.value.data?.smells || MOCK_SMELLS : MOCK_SMELLS);
      });
  }, []);

  useEffect(() => { if (repoLoaded) loadAll(); }, [repoLoaded, loadAll]);
  ...
  const runAll = async () => {
    ...
    setRunStep(0);
    let s = 0;
    const iv = setInterval(() => { s++; setRunStep(s); if (s >= ANALYSIS_STEPS.length) clearInterval(iv); }, 700);
    try {
      await Promise.allSettled([runHealth((selectedRepoId || '1')), runSecurity((selectedRepoId || '1')), runSmells((selectedRepoId || '1'))]);
      await loadAll();
    } catch { /* use mocks */ }
    clearInterval(iv);
    setRunning(false);
              ...
              <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-2xl p-5">
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-zinc-500">Health score, security scan, and code smell detection in one pass.</div>
                  </div>
              ...
              <AnimatePresence mode="wait">
                <motion.div key={tab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                    ...
                    <div className="space-y-5">
                      <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-2xl p-6 flex items-center gap-8">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-zinc-500">helix-backend · Overall score</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="ml-auto flex items-center gap-2">
                          {['critical','high','medium','low'].map(s => {
                            const cfg = SEV[s];
                            const count = security.filter(f => f.severity === s).length;
                            ...
                            <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-xs bg-[#12121a] border border-[#1e1e2e] rounded-lg px-3 py-2">
                                    <span className="text-zinc-500">Fix: </span>{f.suggestion}
                                  </div>
                                </div>
                                <span className={cn('text-[10px] px-2 py-0.5 rounded border flex-shrink-0 capitalize', cfg.bg, cfg.border, cfg.color)}>
                                  {f.severity}
                                </span>
                              </div>
                            </motion.div>
                  ...
                  {tab === 2 && (
                    <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-2xl divide-y divide-[#1e1e2e]">
                      {smells.map((smell, i) => {
                        const cfg = SEV[smell.severity] || SEV['info'];
                        const Icon = cfg.icon;
                        return (
                          <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 }}
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs bg-[#12121a] border border-[#1e1e2e] rounded-lg px-3 py-2">
                                  <span className="text-zinc-500">Fix: </span>{smell.suggestion}
                                </div>
                              </div>
                            </div>
                          </motion.div>
