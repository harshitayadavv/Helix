...
const MOCK_NODES: GraphNode[] = [
  { id: '1', name: 'auth.py', type: 'file', path: 'src/auth.py' },
  { id: '2', name: 'UserService', type: 'class', path: 'src/services.py' },
  { id: '3', name: 'parse_token', type: 'function', path: 'src/utils.py' },
  { id: '4', name: 'validate_user', type: 'function', path: 'src/auth.py' },
  { id: '5', name: 'models.py', type: 'file', path: 'src/models.py' },
  { id: '6', name: 'middleware.py', type: 'file', path: 'src/middleware.py' },
];

const MOCK_IMPACT: ImpactResult = {
  direct_dependents: ['middleware.py', 'AuthHandler', 'validate_user'],
  indirect_dependents: ['app.py', 'UserService', 'Repository'],
  potentially_affected: ['config.py', 'database', 'all API routes'],
  broken_endpoints: ['POST /auth/login', 'POST /auth/logout', 'GET /users/me'],
  risk_score: 'Critical',
  summary: 'Deleting this node would break the authentication pipeline. All protected routes would become inaccessible and 3 API endpoints would return 500 errors.',
};
...
export default function ImpactPage() {
  ...
  useEffect(() => {
    ...
    getGraphNodes((selectedRepoId || '1'))
      .then(r => setNodes(r.data?.nodes || MOCK_NODES))
  ...
  const analyse = useCallback(async () => {
    ...
    setImpact(null);
    try {
      const r = await runImpact((selectedRepoId || '1'), selected.id, selected.type);
      setImpact(r.data || MOCK_IMPACT);
    } catch {
                ...
                <div className="relative flex items-center justify-center" style={{ width: 680, height: 680 }}>
                  <AnimatePresence>
                    {(impact || running) && RINGS.map(({ color, ring }, i) => (
                      <motion.div key={ring}
                        initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: running ? 0.25 : 0.15 }} exit={{ scale: 0, opacity: 0 }}
                        ...
                        return (
                          <motion.div key={`${key}-${ni}`}
                            ...
                            style={{
                              ...
                              background: `${color}15`, borderColor: `${color}40`, color, whiteSpace: 'nowrap',
                            }}>
                            {label}
                          </motion.div>
                        );
