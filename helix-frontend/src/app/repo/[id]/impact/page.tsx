...
const TYPE_ICON: Record<string, typeof FileCode2> = {
  file: FileCode2, function: Code2, class: Box, module: Package,
  File: FileCode2, Function: Code2, Class: Box, Module: Package,
};
...
export default function ImpactPage({ params }: { params: { id: string } }) {
  ...
  .then(r => {
    const raw = r.data?.nodes || r.data || [];
    // Unwrap { n: {...}, labels: [...] } format
    const unwrapped = raw.map((item: any) => {
      if (item.n) {
        const props = item.n;
        ...
        return {
          ...
          path: props.path || props.file_path || '',
        };
      }
      return item;
    }).filter((n: any) => n.id);
  ...
  const IconComponent = selected ? (TYPE_ICON[selected.type] || FileCode2) : FileCode2;

  if (!nodesLoading && nodes.length === 0) {
    return (
      <div className="flex flex-1 min-h-0 items-center justify-center">
        <RepoEmptyState icon={Package} title="No nodes found"
          description="The repository graph hasn't been built yet. Process the repository first." />
      </div>
    );
  }
    ...
    <div className="flex flex-1 min-h-0">
      ...
      <div className="w-60 flex-shrink-0 border-r border-[#1e1e2e] flex flex-col bg-[#0d0d14]">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {nodesLoading
            ? Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-10 bg-[#12121a] rounded mb-1 animate-pulse" />)
            : filtered.map(node => {
                const NIcon = TYPE_ICON[node.type] || FileCode2;
                const color = TYPE_COLOR[node.type] || '#6366f1';
                return (
                  <button key={node.id}
                    onClick={() => { setSelected(node); setImpact(null); }}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={cn('text-xs font-medium truncate',
                        selected?.id === node.id ? 'text-indigo-300' : 'text-zinc-300')}>
                        {node.name}
                      </div>
        ...
        {!selected ? (
          ...
          <div className="relative flex items-center justify-center" style={{ width: 680, height: 680 }}>
            ...
            <AnimatePresence>
              {(impact || running) && RINGS.map(({ color, ring }, i) => (
                <motion.div key={ring}
                  ...
                  animate={{ scale: 1, opacity: running ? 0.25 : 0.15 }}
            ...
            <motion.div
              ...
              style={{
                background: `${TYPE_COLOR[selected.type] || '#6366f1'}15`,
                borderColor: TYPE_COLOR[selected.type] || '#6366f1',
              }}>
              <IconComponent size={24} style={{ color: TYPE_COLOR[selected.type] || '#6366f1' }} />
              ...
              {impact && RINGS.map(({ key, color, ring }, ri) =>
                  ...
                  return (
                    <motion.div key={`${key}-${ni}`}
                      ...
                      style={{
                        transform: `translate(calc(-50% + ${Math.cos(angle) * ring}px), calc(-50% + ${Math.sin(angle) * ring}px))`,
                        background: `${color}15`,
          ...
          <motion.div
            ...
            <div className="p-4 border-b border-[#1e1e2e]">
              ...
              <div className="text-xs text-zinc-500">
                Deleting <span className="text-indigo-300">{selected.name}</span>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
              ...
              <div>
                ...
                <span className={cn('inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border',
                  RISK_CONFIG[impact.risk_score] || RISK_CONFIG.Low)}>
              ...
              {impact.summary && (
                <div>
                  ...
                  <div className="text-xs text-zinc-400 leading-relaxed bg-[#12121a] border border-[#1e1e2e] rounded-lg px-3 py-2">
                    {impact.summary}
                  </div>
                </div>
              )}
