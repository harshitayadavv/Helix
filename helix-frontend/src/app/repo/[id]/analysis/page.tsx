...
interface HealthData { overall: number; grade?: string; sub_scores: SubScore[]; }
...
function getScoreDesc(key: string, score: number): string {
  if (score === 100) return 'No issues detected';
  if (score === 0)   return `No ${key} coverage`;
  if (score >= 80)   return `Good ${key}`;
  if (score >= 50)   return `Some ${key} issues`;
  return `Poor ${key} — needs attention`;
}
  ...
  return (
    <div className="relative w-36 h-36">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        {grade && <span className="text-sm font-semibold text-zinc-400">{grade}</span>}
    ...
    Promise.allSettled([getHealth(params.id), getSecurity(params.id), getSmells(params.id)])
      .then(([h, s, sm]) => {
        ...
        if (h.status === 'fulfilled' && h.value.data) {
          ...
          // Build sub-scores from backend `scores` object
          // Backend: { scores: { architecture: 100, maintainability: 100, ... } }
          const subScores: SubScore[] = d.scores
            ? Object.entries(d.scores).map(([key, value]) => {
                ...
                return {
                  ...
                  desc:  getScoreDesc(key, score),
                };
              })
            : (d.sub_scores || []);

          setHealth({
            overall:    Math.round(d.overall_score ?? d.overall ?? d.score ?? 0),
            grade:      d.grade || '',
        ...
        if (s.status === 'fulfilled' && s.value.data) {
          setSecurity(s.value.data?.findings || s.value.data?.issues || []);
        }
        // ── Smells ──
        if (sm.status === 'fulfilled' && sm.value.data) {
          setSmells(sm.value.data?.smells || sm.value.data?.code_smells || []);
        }
      })
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-5xl mx-auto space-y-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-24 bg-[#0d0d14] border border-[#1e1e2e] rounded-xl animate-pulse" />
          ))}
        </div>
      </div>
    );
          ...
          <motion.div key={tab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              health ? (
                <div className="space-y-5">
                  <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-2xl p-6 flex items-center gap-8">
                    <CircularScore score={health.overall} grade={health.grade} />
                    <div>
                      <div className="text-xl font-bold text-white mb-1">Project Health</div>
                      <div className="text-sm text-zinc-500">Overall score</div>
                      {health.grade && (
                        <div className="mt-2 inline-flex items-center gap-1.5 text-xs text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 rounded-lg px-2.5 py-1">
                          Grade <span className="font-bold text-sm">{health.grade}</span>
                        </div>
                      )}
                    </div>
                  </div>

                  {health.sub_scores.length > 0 && (
                ...
                <RepoEmptyState icon={BarChart2} title="No health data yet"
                  description='Click "Run Analysis" to generate a health score for this repository.'
                  action={{ label: 'Run Analysis', onClick: runAll }} />
              )
            )}
            ...
            {tab === 1 && (
              security.length > 0 ? (
                          ...
                          <button key={s} onClick={() => setFilterSev(filterSev === s ? null : s)}
                            className={cn('text-xs px-2.5 py-1 rounded-lg border transition-colors',
                              filterSev === s
                                ...
                                : 'border-[#1e1e2e] text-zinc-600 hover:text-zinc-400')}>
                            {count} {s}
                          </button>
                        );
                      })}
                      {filterSev && (
                        <button onClick={() => setFilterSev(null)} className="text-xs text-zinc-600 hover:text-zinc-400">clear</button>
                      )}
                    </div>
                  </div>
                ...
                <RepoEmptyState icon={Shield} title="No security findings"
                  description='Run Analysis to scan for security vulnerabilities.' />
              )
            )}
            ...
            {tab === 2 && (
              smells.length > 0 ? (
                ...
                <RepoEmptyState icon={Code2} title="No code smells detected"
                  description='Run Analysis to detect God Classes, long methods, and other patterns.' />
              )
            )}
