...
interface SearchResult { id: string; type: 'function'|'class'|'file'|'api'; name: string; path: string; line_number?: number; description: string; }

const FILTERS: { key: FilterType; label: string }[] = [
  { key: 'all', label: 'All' }, { key: 'function', label: 'Functions' },
  { key: 'class', label: 'Classes' }, { key: 'file', label: 'Files' }, { key: 'api', label: 'APIs' },
];
const TYPE_CONFIG = {
...
export default function SearchPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    getSearchHistory(params.id)
      ...
      .catch(() => {
        try { const s = localStorage.getItem('helix_recent_searches'); if (s) setRecent(JSON.parse(s)); } catch { /* ignore */ }
      });
  }, [params.id]);
  ...
  const doSearch = useCallback(async (q: string, f: FilterType) => {
    ...
    setLoading(true);
    try {
      const r = await searchCode(q, params.id, f);
    ...
    <div className="flex-1 overflow-y-auto px-6 py-10">
      <div className="max-w-2xl mx-auto">
        <div className="relative mb-2">
          ...
          {query && <button onClick={() => setQuery('')} className="absolute right-5 top-1/2 -translate-y-1/2"><X size={16} className="text-zinc-600 hover:text-zinc-400" /></button>}
        </div>
          ...
          {FILTERS.map(({ key, label }) => (
            <button key={key} onClick={() => setFilter(key)}
              className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-colors',
                filter === key ? 'bg-indigo-600/15 text-indigo-300 border-indigo-500/30' : 'text-zinc-500 border-[#1e1e2e] hover:text-zinc-300')}>
              {label}
            </button>
          ))}
        </div>
        ...
        {!query && recent.length > 0 && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] font-medium text-zinc-600 uppercase tracking-wider flex items-center gap-1.5"><Clock size={11} />Recent</span>
              <button onClick={() => setRecent([])} className="text-[11px] text-zinc-600 hover:text-zinc-400">Clear</button>
            </div>
            <div className="flex flex-wrap gap-2">
              {recent.map((r, i) => (
                <button key={i} onClick={() => setQuery(r)}
                  className="px-3 py-1.5 bg-[#0d0d14] border border-[#1e1e2e] rounded-lg text-xs text-zinc-400 hover:text-zinc-200 hover:border-[#2e2e3e] transition-colors">{r}</button>
              ))}
            </div>
          </div>
        )}

        {loading && <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-20 bg-[#0d0d14] border border-[#1e1e2e] rounded-xl animate-pulse" />)}</div>}
          ...
          <div className="space-y-2">
            <AnimatePresence>
              {results.map((result, i) => {
                ...
                const Icon = cfg.icon;
                return (
                  <motion.div key={result.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    onMouseEnter={() => setSelected(i)} onClick={() => viewInGraph(result)}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-[10px] px-1.5 py-0.5 rounded uppercase tracking-wide" style={{ background: `${cfg.color}15`, color: cfg.color }}>{result.type}</span>
                      </div>
                      <div className="text-xs text-zinc-600 font-mono mb-1">{result.path}{result.line_number ? `:${result.line_number}` : ''}</div>
        ...
        {results.length > 0 && <div className="flex items-center gap-4 mt-6 text-[10px] text-zinc-700 justify-center"><span>↑↓ navigate</span><span>↵ open in graph</span></div>}
      </div>
    </div>
  );
}
